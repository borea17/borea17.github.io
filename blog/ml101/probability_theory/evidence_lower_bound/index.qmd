...
distribution $p_{\boldsymbol{\theta}} (\textbf{z})$ and then
$\textbf{x}^{(i)}$ is sampled from a conditional distribution
$p_{\boldsymbol{\theta}} \left(\textbf{x} | \textbf{z}^{(i)}\right)$.

The **evidence lower bound (ELBO)** $\mathcal{L} \left(
  \boldsymbol{\theta}, \boldsymbol{\phi}; \textbf{x}^{(i)}\right)$ (or **variational lower
bound**) which is
...
p_{\boldsymbol{\theta}}(\textbf{x}^{(i)})$ **given a variational
approximation** $q_{\boldsymbol{\phi}} \left(\textbf{z} |
...
\end{align}
$$
where $p_{\boldsymbol{\theta}} (\textbf{z})$ is a prior on the latent
distribution. Note that true posterior $p_{\boldsymbol{\theta}} \left( \textbf{z} |
...
true posterior distribution $p_{\boldsymbol{\theta}} \left( \textbf{z} |
...
$q_{\boldsymbol{\phi}} \left(\textbf{z} | \textbf{x} \right)$ and a
latent prior $p_{\boldsymbol{\theta}} (\textbf{z})$. This is the main
