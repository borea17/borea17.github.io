...
There is **no upper or lower bound for covariance** which can be seen by simply scaling one 
distribution. Note that scaling would also result in an increased variance within the scaled 
distribution.
...
If you looked into the generation of the visualization above, you saw that we generated the different plots 
via fixing the covariance $\text{Cov}(\textbf{X}, \textbf{Y})$ and then sampling from a 
multivariate normal distribution $\mathcal{N}\big(\boldsymbol{\mu}, \boldsymbol{\Sigma}\big)$

In fact, we weren't only fixing $\text{Cov}(\textbf{X}, \textbf{Y})$, but rather the
**distributional parameters** of $\mathcal{N}\big(\boldsymbol{\mu}, \boldsymbol{\Sigma}\big)$

* _mean vector_ $\boldsymbol{\mu}$ representing the **center of the distribution** which we set to 
  $\boldsymbol{\mu} = \begin{bmatrix} 0 & 0\end{bmatrix}^{\text{T}}$,
* _covariance matrix_ $\boldsymbol{\Sigma}$ representing the **shape of the distribution**. It's entries are defined 
  as follows
...
[
  Inputs.range([-2, 2], {label: tex`\mu_{\textbf{X}}`, step: 0.1}),
  Inputs.range([-2, 2], {label: tex`\mu_{\textbf{Y}}`, step: 0.1}),
  Inputs.range([1, 2], {label: tex`\text{Cov}\big(\textbf{X}, \textbf{X}\big)`, step: 0.1}),
  Inputs.range([0, 1], {label: tex`\text{Cov}\big(\textbf{X}, \textbf{Y}\big)`, step: 0.1}),
  Inputs.range([1, 2], {label: tex`\text{Cov}\big(\textbf{Y}, \textbf{Y}\big)`, step: 0.1}),
])
...
Plot.plot({
  ...
  marks: [
    ...
    // plot eigenvector starting from mean
...
<!-- import { Matrix, Distribution } from "@ml-matrix" -->
<!---->
<!-- distributionParams = { -->
<!--   const dist = new Distribution.MultivariateNormal(meanVector, covarianceMatrix); -->
<!-- } -->

<!-- import { MultivariateNormal } from "./multivariate-normal.js" -->
<!---->
<!-- MultivariateNormal(meanVector, covarianceMatrix) -->

<!-- ```{ojs} -->
<!-- //| fig-cap: "Interactive Visualization of a Multivariate Gaussian" -->
<!---->
<!-- ``` -->
