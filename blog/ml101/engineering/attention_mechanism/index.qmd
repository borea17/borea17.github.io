...
[Attention is all you need](https://arxiv.org/abs/1706.03762) paper, the **attention weights are 
...
$$
\begin{align*}
&\textbf{a}_i = \sum_j w_{ij} \textbf{v}_j \quad \text{with} \quad w_{ij} \ge 0 \quad \text{and} \quad \sum_j w_{ij} = 1,\\
&\text{since} \quad w_{ij} = \text{Softmax} \left( \textbf{q}_i, \textbf{K} \right) =
\frac {\exp \left(\textbf{q}_i^{\text{T}} \textbf{k}_j \right)}
{\sum_{j} \exp \left( \textbf{q}_i^{\text{T}} \textbf{k}_j \right)}.
\end{align*}
$$
