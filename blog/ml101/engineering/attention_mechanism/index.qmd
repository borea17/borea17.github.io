...
Note that without $\textbf{W}^O \in \mathbb{R}^{ hd_v \times d_{\text{model}}}$ and adapted linear projections using 
the folowing dimensions $\textbf{W}_i^Q \in \mathbb{R}^{ d_{k} \times d_k}, 
...
different representations of the query, key and value matrices (otherwise each head would look exactly the same)..

Using $d_{\text{model}}$ instead of $d_k / d_v$ for the linear projections is an architectural decision 
to reduce the number of trainable paramters while whithout losing too much expressiveness. The concatenation plus linear 
layer increases the expresiveness of the model, since it allows the model to combine attention representations of different subspaces.

| ![Multi-Head Attention](./img/multi_head_att.png "Multi-Head Attention") |
| :--         |
| **Multi-Head Attention Layer**: Linearly project values, queries and keys $h$-times into model dimension, apply scaled-dot-product attention, concatenate result (attention in different subspaces) and linearly project once again (simply increase the expressiveness). Taken from [Vaswani et al. (2017)](https://arxiv.org/abs/1706.03762). |

Below is a simple implementation of multi-head attention using the masked attention implementation 
above.
